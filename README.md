...
> GitHub Releases에서 작은 launcher `kras_quick_v1.0.5.exe`를 받아 SHA-256 검증 후 `Downloads\kras-quick.exe`로 원자적으로 교체하고 실행합니다. 첫 실행 때 runtime ZIP(약 44MB)을 한 번 내려받아 `%LOCALAPPDATA%\kras-quick`에 캐시합니다. 설치 스크립트 원문: [`install.ps1`](install.ps1)
